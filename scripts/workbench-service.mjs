...
createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === "POST" && request.url === "/api/projects/artifacts") return respond(response, 200, await service.listArtifacts((await readJson(request)).project));
    if (request.method === "POST" && request.url === "/api/projects/candidates") { const body = await readJson(request); return respond(response, 200, await service.readCandidates(body.project, body.stepId)); }
