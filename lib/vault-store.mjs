...
const CANDIDATE_STEPS = new Set(["cases", "angle", "title"]);
...
export function createVaultStore({ vaultRoot, workspaceRelativePath = DEFAULT_WORKSPACE }) {
  ...
  function projectDirectory(project) {
    ...
    const pathWithinVault = relative(root, target);
    if (!pathWithinVault || pathWithinVault === ".." || pathWithinVault.startsWith(`..${sep}`)) {
      throw new Error("Project path must remain within the vault");
    }
    return target;
  }
  ...
  return {
    ...
    async listProjects() {
      try {
        ...
        const projects = await Promise.all(entries.filter((entry) => entry.isDirectory() && !entry.name.startsWith(".")).map((entry) => this.getProject(entry.name)));
    ...
    async readCandidates(project, stepId) {
      if (!CANDIDATE_STEPS.has(stepId)) throw new Error("Unsupported candidate step");
      const sourcePath = join(".workbench", `${stepId}.json`);
      try {
        const manifest = JSON.parse(await readFile(join(projectDirectory(project), sourcePath), "utf8"));
        return { sourcePath, candidates: Array.isArray(manifest.candidates) ? manifest.candidates : [] };
      } catch (error) {
        if (error.code === "ENOENT") return { sourcePath, candidates: [] };
        throw error;
      }
    },
