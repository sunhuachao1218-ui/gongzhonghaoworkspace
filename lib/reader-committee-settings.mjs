...
export function createReaderCommitteeSettingsStore({ envPath }) {
  ...
  return {
    ...
    async listModels(fetchImpl = globalThis.fetch) {
      ...
      const sixMonthsAgo = Math.floor((Date.now() - 183 * 24 * 60 * 60 * 1000) / 1000);
      return (Array.isArray(data.data) ? data.data : []).filter((item) => clean(item.id) && Number(item.created) >= sixMonthsAgo).map((item) => ({ id: clean(item.id), name: clean(item.name) || clean(item.id), created: Number(item.created) })).sort((a, b) => b.created - a.created || a.name.localeCompare(b.name));
    },
  };
}
