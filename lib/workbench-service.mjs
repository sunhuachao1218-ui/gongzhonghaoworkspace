    ...
    async runStep({ project, step, skill, input }) {
      ...
      const response = await fetchImpl(`${normalizeUrl(hermesUrl)}/v1/runs`, {
        ...
        body: JSON.stringify({
          ...
          input: `使用 Hermes Skill ${skill.name}${skill.version ? ` ${skill.version}` : ""} 执行“${step.label}”。Skill 是本步骤业务规则的唯一来源。项目元数据：${JSON.stringify({ id: project.id, title: project.title, obsidianPath: project.obsidianPath })}。仅将内容和来源写入该 Obsidian 项目目录；工作台只接收版本、状态与文件路径。用户输入：${input || "无"}`,
        }),
      });
