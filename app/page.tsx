...
export default function Home() {
  ...
  const [, setSkillMapRevision] = useState(0);
  ...
  const open = (id) => { const item = projects.find((p) => p.id === id); localStorage.setItem("gongzhonghao-workbench-view", JSON.stringify({ projectId: id, activeStep: item.currentStep })); setProjectId(id); setActiveStep(item.currentStep); setChoice(""); };
  ...
  useEffect(() => {
    let savedProjects;
    let savedView;
    try { savedProjects = JSON.parse(localStorage.getItem("gongzhonghao-workbench-projects") || "null"); } catch { savedProjects = null; }
    try { savedView = JSON.parse(localStorage.getItem("gongzhonghao-workbench-view") || "null"); } catch { savedView = null; }
    const nextProjects = ensureCoverTestProject(savedProjects || INITIAL_PROJECTS);
    const restoredView = restoreView(nextProjects, savedView);
    setProjects(nextProjects);
    setProjectId(restoredView.projectId);
    setActiveStep(restoredView.activeStep);
    setHasLoadedLocalProjects(true);
  }, []);
  useEffect(() => {
    try {
      const savedOverrides = JSON.parse(localStorage.getItem("gongzhonghao-workbench-skill-map") || "{}");
      Object.assign(SKILL_MAP, mergeSkillMap(SKILL_MAP, savedOverrides));
      setSkillMapRevision((revision) => revision + 1);
    } catch { /* Ignore malformed local settings and keep the configured defaults. */ }
  }, []);
  ...
  const loadCandidates = async (stepId = activeStep, targetProject = project) => {
    ...
    setCandidateResults((items) => ({ ...items, [stepId]: result.candidates }));
  ...
  const requestHermesRun = async () => {
    ...
    if (skill.name === "待配置") {
      ...
      const name = window.prompt(`请填写“${stepLabel}”要调用的 Hermes Skill 名称`);
      if (!name?.trim()) return;
      const version = window.prompt("Skill 版本（可留空）", "—");
      skill = { ...skill, name: name.trim(), version: version?.trim() || "—" };
      Object.assign(SKILL_MAP, mergeSkillMap(SKILL_MAP, { [activeStep]: skill }));
      localStorage.setItem("gongzhonghao-workbench-skill-map", JSON.stringify({ [activeStep]: skill }));
      setSkillMapRevision((revision) => revision + 1);
    }
    ...
    try {
      ...
      const poll = async () => {
        ...
        if (["completed", "succeeded"].includes(status.status)) {
          ...
          setIsRunning(false);
          setRunMessage(candidates.length ? `Hermes 已完成，已载入 ${candidates.length} 个候选。请选定后确认。` : generated ? `Hermes 已完成，已识别 Obsidian 文件：${generated.path}。请核对后确认本版本。` : "Hermes 已完成，但当前项目目录未发现本次新增的 Markdown 文件；请在 Obsidian 核对后再确认。");
          return;
        }
  ...
  if (project) return <main className="min-h-screen bg-[#f5f2ec] p-4 text-[#20211e] sm:p-8">
    ...
    <div className="mx-auto max-w-7xl"><p className="text-xs font-semibold tracking-[.18em] text-[#7b8579]">文章项目</p><h1 className="mt-2 max-w-3xl text-3xl font-semibold tracking-tight">{project.title}</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]"><section className="rounded-2xl border border-[#dfdad0] bg-white p-6 shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="text-sm text-[#687269]">第 {STEPS.findIndex(([id]) => id === activeStep) + 1} 步</p><h2 className="text-2xl font-semibold">{STEPS.find(([id]) => id === activeStep)?.[1]}</h2></div><span className="rounded-full bg-[#e9efe9] px-3 py-1 text-sm text-[#315841]">{statusText[step.status]}</span></div>
        ...
        {["cases", "angle", "title"].includes(activeStep) && <div className="mt-6"><div className="flex items-center justify-between gap-3"><p className="font-medium">{activeStep === "cases" ? `候选池 · 第 ${caseRound} 轮 · 勾选后收集一个主案例` : activeStep === "angle" ? "同一案例的候选角度 · 只选一个" : "候选标题 · 选一个锁定"}</p><button onClick={() => loadCandidates().catch((error) => setRunMessage(error instanceof Error ? error.message : "读取 Hermes 候选失败"))} className="text-sm text-[#315841]">刷新候选</button></div><div className="mt-3 grid gap-3 sm:grid-cols-2">{displayedCandidates.map((candidate) => <label key={candidate.id} className={`cursor-pointer rounded-xl border p-4 ${choice === candidate.title ? "border-[#386448] bg-[#f0f6ef]" : "border-[#e2ded6]"}`}><input className="mr-2" type="radio" name="choice" checked={choice === candidate.title} onChange={() => setChoice(candidate.title)} /><span className="font-medium">{candidate.title}</span>{candidate.summary && <span className="mt-2 block text-sm text-[#59635a]">{candidate.summary}</span>}{activeStep === "cases" && <span className="mt-3 block border-t border-[#dfe5df] pt-3 text-xs leading-5 text-[#5d6a60]">{candidate.fit && <>匹配：{candidate.fit}<br/></>}{candidate.source && <>来源：{candidate.source}<br/></>}{candidate.credibility && <>可信度：{candidate.credibility}<br/></>}{candidate.evidenceRisk && <>证据风险：{candidate.evidenceRisk}</>}{!candidate.fit && !candidate.source && !candidate.credibility && !candidate.evidenceRisk && "匹配点、可支持方向、原始来源、来源可信度与证据风险，将由 Hermes 检索后写入 Obsidian；工作台只展示引用路径和确认状态。"}</span>}</label>)}</div>{activeStep === "cases" && <div className="mt-4 flex flex-wrap gap-3"><button onClick={() => { setChoice(""); setCaseRound((round) => round + 1); }} className="rounded-lg border border-[#b9c7bb] px-3 py-2 text-sm">重新搜索候选</button><button disabled={!choice} onClick={() => mutate((p) => selectStepValue(p, "cases", choice))} className="rounded-lg bg-[#e6f0e8] px-3 py-2 text-sm font-medium text-[#315841] disabled:opacity-40">收集已选案例</button></div>}</div>}
        ...
        <div className="mt-7 flex flex-wrap gap-3"><button disabled={!integration.hermes || isRunning} onClick={requestHermesRun} className="rounded-lg border px-4 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-45">{isRunning ? "Hermes 处理中…" : ["outline", "draft"].includes(activeStep) ? "调用 Hermes 重写" : "调用 Hermes 生成"}</button><button onClick={() => mutate((p) => confirmStep(["topic", "cases", "angle", "title"].includes(activeStep) && choice ? selectStepValue(p, activeStep, choice) : p, activeStep))} className="rounded-lg bg-[#1f4733] px-4 py-2 text-sm text-white">确认当前版本</button><button onClick={() => mutate((p) => requestRevision(p, activeStep))} className="rounded-lg px-4 py-2 text-sm text-[#8b3b31]">返回修改</button></div>{runMessage && <p className="mt-3 text-sm text-[#556b5d]">{runMessage}</p>}
