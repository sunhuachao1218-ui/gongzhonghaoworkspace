...
import { STEPS, addVersion, canOpenStep, confirmStep, createProject, requestRevision, selectDefaultStyle, selectStepValue } from "../lib/workflow.js";
...
export default function Home() {
  ...
  if (project) return <main className="min-h-screen bg-[#f5f2ec] p-4 text-[#20211e] sm:p-8">
    ...
    <div className="mx-auto max-w-7xl"><p className="text-xs font-semibold tracking-[.18em] text-[#7b8579]">文章项目</p><h1 className="mt-2 max-w-3xl text-3xl font-semibold tracking-tight">{project.title}</h1>
      <section className="my-7 grid gap-2 rounded-2xl border border-[#dfdad0] bg-white p-4 shadow-sm lg:grid-cols-9">{STEPS.map(([id, label], i) => <button key={id} disabled={!canOpenStep(project, id)} onClick={() => setActiveStep(id)} className={`flex min-w-0 min-h-32 flex-col justify-between rounded-xl p-3 text-left ${activeStep === id ? "bg-[#1f4733] text-white" : "bg-[#f6f4ef]"} disabled:opacity-45`}><span className="block text-sm opacity-70">{i + 1}.</span><span className="flex items-center gap-2 whitespace-nowrap text-lg font-semibold leading-tight"><span>{label}</span><span className="text-base leading-none opacity-75">{project.steps[id].status === "confirmed" ? "✓" : "○"}</span></span></button>)}</section>
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]"><section className="rounded-2xl border border-[#dfdad0] bg-white p-6 shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="text-sm text-[#687269]">第 {STEPS.findIndex(([id]) => id === activeStep) + 1} 步</p><h2 className="text-2xl font-semibold">{STEPS.find(([id]) => id === activeStep)?.[1]}</h2></div><span className="rounded-full bg-[#e9efe9] px-3 py-1 text-sm text-[#315841]">{statusText[step.status]}</span></div>
        ...
        {["layout", "cover"].includes(activeStep) && <div className="mt-6 rounded-xl border border-dashed border-[#c8d1c9] p-4"><p className="font-medium">默认风格</p><p className="mt-1 text-sm text-[#687269]">首次选择后复用；第一阶段只保存偏好，不进行真实渲染或生成。</p><p className="mt-2 text-sm font-medium text-[#315841]">当前：{activeStep === "layout" ? project.layoutStyle : project.coverStyle}</p><button onClick={chooseDefaultStyle} className="mt-3 rounded-lg border px-3 py-2 text-sm">选择默认风格</button></div>}
