...
export default function Home() {
  ...
  const loadModels = async () => { setSettingsMessage("正在读取 OpenRouter 模型…"); try { const response = await fetch("http://127.0.0.1:4174/api/reader-committee/models"); const models = await response.json(); if (!response.ok) throw new Error(models.error || "读取失败"); setAvailableModels(models); setSettingsMessage(`已读取 ${models.length} 个模型`); } catch (error) { setSettingsMessage(error instanceof Error ? error.message : "模型列表读取失败"); } };
