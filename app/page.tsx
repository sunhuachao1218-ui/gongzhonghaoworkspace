...
export default function Home() {
  ...
  if (project) return <main className="min-h-screen bg-[#f5f2ec] p-4 text-[#20211e] sm:p-8">
    ...
    <div className="mx-auto max-w-7xl"><p className="text-xs font-semibold tracking-[.18em] text-[#7b8579]">文章项目</p><h1 className="mt-2 max-w-3xl text-3xl font-semibold tracking-tight">{project.title}</h1>
      ...
      </section><aside className="space-y-4"><div className="rounded-2xl border border-[#dfdad0] bg-white p-5"><h3 className="font-semibold">文章总控</h3>{[["主题", project.title],["当前案例", project.mainCase],["已选角度", project.angle],["当前标题", project.lockedTitle],["当前步骤", STEPS.find(([id]) => id === project.currentStep)?.[1]],["Obsidian", project.obsidianPath]].map(([k,v]) => <div className="mt-4" key={k}><p className="text-xs text-[#758078]">{k}</p><p className="mt-1 text-sm">{v}</p></div>)}</div><div className="rounded-2xl border border-[#dfdad0] bg-white p-5"><h3 className="font-semibold">版本记录</h3>{step.versions.length ? step.versions.map((v) => <p key={v} className="mt-3 rounded-lg bg-[#f6f4ef] px-3 py-2 text-sm">{v}{step.confirmedVersion === v ? " · 已确认" : " · 待确认"}</p>) : <p className="mt-3 text-sm text-[#758078]">尚无版本</p>}</div></aside></div></div>{samplePreview && <div role="dialog" aria-modal="true" aria-label={`${samplePreview.heading}样图大图`} className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-5" onClick={() => setSamplePreview(null)}><div className="relative w-full max-w-5xl" onClick={(event) => event.stopPropagation()}>{samplePreview.image ? <img src={samplePreview.image} alt={`${samplePreview.heading}样图：${samplePreview.label}`} className="max-h-[85vh] w-full rounded-2xl object-contain shadow-2xl" /> : <div className="flex h-72 items-center justify-center rounded-2xl bg-[#f4f1ea] text-[#687269]">暂无样图</div>}<div className="absolute left-4 top-4 rounded-lg bg-black/45 px-3 py-2 text-sm text-white backdrop-blur-sm"><p className="font-medium">{samplePreview.heading} · {samplePreview.label}</p><p className="mt-0.5 text-white/75">{previewSamples.findIndex((option) => option.id === samplePreview.id) + 1} / {previewSamples.length}</p></div><button type="button" onClick={() => setSamplePreview(null)} className="absolute right-4 top-4 rounded-full bg-black/45 px-3 py-2 text-sm text-white backdrop-blur-sm transition hover:bg-black/65">关闭</button><button type="button" aria-label="上一张样图" onClick={() => moveSamplePreview(-1)} className="absolute left-4 top-1/2 -translate-y-1/2 rounded-full bg-black/45 px-4 py-3 text-xl text-white backdrop-blur-sm transition hover:bg-black/65">←</button><button type="button" aria-label="下一张样图" onClick={() => moveSamplePreview(1)} className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full bg-black/45 px-4 py-3 text-xl text-white backdrop-blur-sm transition hover:bg-black/65">→</button></div></div>}</main>;
